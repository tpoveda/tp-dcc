...
def filter_shapes(
    ...
    """
    If a shape node is found in given objects, it will be replaced with its transform parent. Otherwise, will leave
    the node in the list.

    :param objs_list: list of Maya object names
    :param allow_joints: Whether to include joints in the returned list or not. They will be treated as
        transform nodes.
    :param allow_dg: Whether to return DG nodes in the returned list of not.
    :return: list of Maya object names with only transform nodes.
    :raises ValueError: If the parent of a shape node cannot be found.
    """
...
def filter_all_node_types(
    ...
    """
    Function that allow filter nodes with no restrictions of node types.

    :param selection_only: Whether to search all scene objects or only selected ones in the scene.
    :param search_hierarchy: Whether to search objects in hierarchies.
    :param dag: Whether to return only DAG nodes.
    :param remove_maya_defaults: Whether to ignore Maya default nodes.
    :param transforms_only: Whether to return only transform nodes.
    :return: list of node names in Maya that matches the given criteria.
    """

    if not selection_only:
        all_objs = cmds.ls(long=True, dagObjects=search_hierarchy, dag=dag)
    ...
    all_selected_objs = cmds.ls(
        selection=True, long=True, dagObjects=search_hierarchy, dag=dag
    )
...
def filter_nodes_by_type(
    ...
    :param filter_types: list of node types to filter by.
    :param objs_list: list of maya nodes to filter.
    :param search_hierarchy: whether to search objects in hierarchies
    :param search_scene: whether to search all scene objects.
    :param dag: whether to return only DAG nodes
    :return: list of filtered objects that match the filter criteria.
    """
    ...
    if not search_scene:
        for filter_type in filter_types:
            filtered_objs += cmds.ls(
                type=filter_type, long=True, dagObjects=search_hierarchy, dag=dag
            )
    else:
        for filter_type in filter_types:
            filtered_objs += cmds.ls(
...
def filter_by_group(
    selection_only: bool, search_hierarchy: bool, dag: bool
) -> list[str]:
    """
    Returns nodes that are groups (empty transform nodes with no shape nodes).

    :param selection_only: whether to search all scene objects or only selected ones
    :param search_hierarchy: whether to search the hierarchy below the objects list or not
    :param dag: whether to only return DAG nodes or not
    :return: list of object that matches the filter criteria.
    """
    ...
    if selection_only:
        objs_list = cmds.ls(sl=True, long=True)
        if not objs_list:
...
def filter_dag_transforms(
    ...
    """
    Returns a list of Maya nodes that match filter type and returns only transform parent of shape nodes.

    :param filter_types: list of node types to filter by.
    :param search_hierarchy: whether to search objects in hierarchies.
    :param selection_only: whether to search all scene objects or only selected ones.
    :param dag: whether to return only DAG nodes.
    :param transforms_only: whether to return only transform nodes or not.
    :return: list of filtered DAG transform nodes.
    """

    if selection_only:
        sel_objs = cmds.ls(sl=True, long=True)
        if not sel_objs:
...
def filter_by_type(
    ...
    """
    Returns a list of objects that match the given node type. This function allows:
        - Filter by a list of node types.
        - Filter by special node types (such as "groups", which does not have a node type name).
    ...
    Notes:
        - Filter types include types that are not Maya object types such as "Groups" and "All types'.
        - Some types are multiple object types, "Light" for example will find many node types.

    :param filter_type: Type that describes a node type in Maya. This should match a string from the TYPE_FILTERS keys.
    :param search_hierarchy: Whether to search objects in hierarchies.
    :param selection_only: Whether to search all scene objects or only selected ones in the scene.
    :param dag: Whether to return only DAG nodes.
    :param remove_maya_defaults: Whether to ignore Maya default nodes.
    :param transforms_only: Whether to return only transform nodes.
    :param include_constraints: Whether to include constraints in the returned list.
    :param keep_order: Whether to take into account the order of selection and hierarchy.
    :return: list of node names in Maya that matches the given criteria.
    :raises ValueError: If the filter type is not found in the TYPE_FILTERS dictionary.
    """
    ...
    if keep_order:
        if selection_only:
            sel_objs = cmds.ls(sl=True, long=True, dagObjects=search_hierarchy)
