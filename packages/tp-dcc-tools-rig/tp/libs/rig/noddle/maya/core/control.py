...
class Control(api.DagNode):
    ...
    def create(
        ...
        group_node = api.factory.create_dag_node(naming.generate_name(name, side, suffix='grp'), 'transform', parent=temp_parent)
        ...
        if offset_group:
            offset_node = api.factory.create_dag_node(naming.generate_name(name, side, suffix='ofs'), 'transform', parent=temp_parent)
            temp_parent = offset_node

        transform_node = api.factory.create_dag_node(naming.generate_name(name, side, suffix='ctl'), 'transform', parent=temp_parent)
        temp_parent = transform_node

        if joint:
            control_joint = api.factory.create_dag_node(naming.generate_name(name, side, suffix='cjnt'), 'joint', parent=temp_parent)
