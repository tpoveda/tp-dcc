...
"""
Module that contains tpRigToolkit Maya plugins loader functions
"""
...
def load_base_loc_plugin(reload=False, debug=False):
	...
	plugin_name = 'baseLoc_maya{}.mll'.format(dcc.get_version())
	plugin_path = path.join_path(get_root_path(), 'baseLoc', 'plug-in', dcc.get_version(), 'Debug' if debug else 'Release')
...
def _load_plugin(plugin_name, plugin_path=None, reload=False, add_to_trusted_paths=True):
	...
	try:
		...
		if not dcc.is_plugin_loaded(plugin_name):
			dcc.load_plugin(plugin_name)
		elif dcc.is_plugin_loaded(plugin_name) and reload:
			dcc.unload_plugin(os.path.basename(plugin_name))
			dcc.load_plugin(plugin_name)
