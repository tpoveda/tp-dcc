...
from tp.tools.rig.noddle.builder.graph.core import socket, edge
from tp.tools.rig.noddle.builder.graph.graphics import socket as socket_view
...
class EdgeDrag:
        ...
        try:
            ...
            if isinstance(item.socket, socket.OutputSocket):
                ...
                self._drag_edge = edge.Edge(
                    self._graphics_view.scene, start_socket=item.socket, end_socket=None, silent=True)
            else:
                logger.debug('Assign end socket to: {0}'.format(item.socket))
                self._drag_edge = edge.Edge(
                    self._graphics_view.scene, start_socket=None, end_socket=item.socket, silent=True)
        except Exception:
    ...
    def end_edge_drag(self, item: qt.QGraphicsItem) -> bool:
        ...
        try:
            new_edge = edge.Edge(self._graphics_view.scene, start_socket=start_socket, end_socket=end_socket)
            self._drag_start_socket = None
            logger.debug(f'EdgeDrag: created new edge {new_edge.start_socket} -> {new_edge.end_socket}')
            self._graphics_view.scene.history.store_history('Edge created by dragging', set_modified=True)
            return True
        except Exception:
            self._drag_start_socket = None
