...
def mixin(cls):
    """
    Decorator that can be used to allow custom widget classes to retrieve theme info.
    ...
    def my__init__(self, *args, **kwargs):
        ...
        self._theme_preferences.updated.connect(self.update_theme)
        self._theme_updates = kwargs.get('theme_updates', True)
    ...
    def theme(self):
        return self._theme_preferences.current_theme()

    def theme_data(self):
        return self._theme_preferences.theme_data()

    def theme_stylesheet(self):
        stylesheet = self._theme_preferences.stylesheet()
        if not stylesheet:
            return ''
    ...
    def theme_default_size(self):
        current_theme_data = self.theme_data()
        return Theme.DEFAULT_SIZE if not current_theme_data else current_theme_data.default_size

    def background_color(self, as_qcolor=False):
        current_theme_data = self.theme_data()
        bg_color = Theme.Colors.DARK_BACKGROUND_COLOR if not current_theme_data else current_theme_data.MAIN_BACKGROUND_COLOR

        return qt_color.color_from_string(bg_color) if as_qcolor else bg_color

    def accent_color(self):
        current_theme_data = self.theme_data()
        return Theme.Colors.BLUE if not current_theme_data else current_theme_data.ACCENT_COLOR

    def set_theme_updates(self, flag):
        """
        Sets whether or not theme changes will be applied to this widget.
        :param bool flag: True to enable theme updates; False otherwise.
        """

        self._theme_updates = flag

    def update_theme(self, event=None):
        """
        Function that is called when the theme is updated.

        :param ThemeUpdateEvent event: custom event that contains info of the updated theme.

        ..note:: this function can be implemented by each widget.
        """
    ...
    setattr(cls, 'background_color', background_color)
    setattr(cls, 'accent_color', accent_color)
    setattr(cls, 'theme_data', theme_data)
    setattr(cls, 'theme_stylesheet', theme_stylesheet)
    setattr(cls, 'set_theme_updates', set_theme_updates)
    setattr(cls, 'update_theme', update_theme)
...
class Theme(dict):
    ...
    def _init_default_options(self):
        ...
        data = dict(
            dpi=1,
            unit='px',
            PRIMARY_FONT=qt.font_db().get_primary_font(consts.Sizes.MEDIUM_FONT_SIZE)[0].family(),
            SECONDARY_FONT=qt.font_db().get_secondary_font(consts.Sizes.SMALL_FONT_SIZE)[0].family()
        )
