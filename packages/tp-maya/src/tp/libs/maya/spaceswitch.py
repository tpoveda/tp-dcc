...
class MatrixConstraint(Constraint):
    ...
    def _build_offset_parent_matrix_constraint(
        ...
        **constraint_kwargs: dict[str, Any],
        ...
        if any((skip_translate, skip_rotate, skip_scale)):
    ...
    def _build_matrix_constraint(
        ...
        **constraint_kwargs: dict[str, Any],
        ...
        def _compose_joint_matrix_rotation_graph() -> list[DGNode]:
            ...
            driven.rotateOrder.connect(_decompose.inputRotateOrder)
        ...
        def _compose_joint_matrix_translate_scale_graph() -> list[DGNode]:
            ...
            _mult_matrix = factory.create_mult_matrix(
                name + "_outputTSMat", inputs=_inputs, output=None
            )
            _decompose = factory.create_decompose(
        ...
        maintain_offset = constraint_kwargs.get("maintainOffset", False)
        skip_translate = constraint_kwargs.get("skipTranslate", (True, True, True))
        skip_rotate = constraint_kwargs.get("skipRotate", (True, True, True))
        skip_scale = constraint_kwargs.get("skipScale", (True, True, True))
        name = driven.fullPathName(partial_name=True, include_namespace=False)
        target_info = drivers["targets"]
        _, target_nodes = zip(*target_info)
        driver = target_nodes[0]
...
def build_constraint(
    ...
    **kwargs,
    ...
    """Builds a space switching ready constraint. Supported constraints are:
        ...
        - Orient Constraint.
        - Scale Constraint.
        - Matrix Constraint (not supported for space switching, just for regular constraints).
    ...
    .. code-block:: python
        ...
        for n in ("locator1", "locator2", "locator3"):
            targets.append((n, factory.create_dag_node(n, "locator")))
        space_node = factory.create_dag_node("control", "locator")
        driven_node = factory.create_dag_node("driven", "locator")

        spaces = {
            "spaceNode": space_node,
            "attributeName": "parentSpace",
            "targets": targets
        }
        constraint, utils = spaceswitch.build_constraint(driven_node, drivers=spaces)

        spaces = {
            "spaceNode": space_node,
            "attributeName": "parentSpace",
            "targets": (("locator8", factory.create_dag_node("locator8", "locator")),)
        }
        constraint, utils = spaceswitch.build_constraint(driven_node, drivers=spaces)
    ...
    :param drivers: a dict containing the target information. e.g:
        {
            'targets': (
                (driver_guide.fullPathName(partial_name=True,
                include_namespace=False), driver_guide),
            )
        }
    :param constraint_type: constraint type.
    :param track: whether the constraint and all nodes created should be tracked
        via metadata.
    :keyword maintainOffset: whether to maintain offset transformation after
