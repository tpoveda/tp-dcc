...
def global_log_level_override(logger):
	"""
	Makes sure the global level override is setup.

	:param logging.Logger logger: logger to setup.
	"""
...
class _MetaLogHandler(type):
	def __call__(cls, *args, **kwargs):
		...
		if 'cmds' in main.__dict__:
			from tp.maya.cmds import logger
			if as_class:
				return logger.MayaLogHandler
			else:
				return type.__call__(logger.MayaLogHandler, *args, **kwargs)
		else:
			return None
...
def create_logger():
	...
	logging_config = os.path.normpath(os.path.join(os.path.dirname(root_path), 'core', '__logging__.ini'))
