"""Token and KeyValue classes for naming conventions."""
...
class Token:
    ...
    def padding(self) -> int:
        """Returns the number of digits for zero-padding numeric values.

        Returns:
            int: Number of digits for padding. If 0, no padding is applied.
        """
    ...
    def padding(self, value: int):
        """Sets the number of digits for zero-padding numeric values.

        Args:
            value (int): Number of digits for padding. Must be >= 0.
        """
    ...
    def _apply_padding(self, value: str | None) -> str | None:
        """Apply zero-padding to numeric values if padding is configured.

        Args:
            value (str or None): Value to potentially pad.

        Returns:
            str or None: Padded value if numeric and padding is set, otherwise original value.
        """

        if value is None or self._padding <= 0:
            return value

        # Try to interpret as integer and apply padding
        try:
            numeric_value = int(value)
            return str(numeric_value).zfill(self._padding)
    ...
    def solve(self, key: str | None, default_value: str | None = None) -> str:
        ...
        if not key:
            token = self.default
            value = token.value if token else default_value
            return self._apply_padding(value)

        for token in self._token_values:
            if token.name == key:
                return self._apply_padding(token.value)

        # Key not found in token table, apply padding to the key itself (raw value)
        return self._apply_padding(key) if self._padding > 0 else default_value
    ...
    def to_dict(self) -> dict:
        ...
        # Only include padding if it's set (non-zero)
        if self._padding > 0:
            result["padding"] = self._padding
        # Only include strict if it's True
        if self._strict:
            result["strict"] = self._strict
        return result
