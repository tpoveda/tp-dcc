...
from tp.libs.rig.noddle.maya.functions import attributes, naming, transforms, joints, nodes, rig
...
class FKIKComponent(animcomponent.AnimComponent):
    ...
    def setup(
        ...
        joint_offset_group = nodes.create(
            'transform', [self.indexed_name(), 'constr'], self.side(), suffix='grp',
            p=self.joints_group().fullPathName())
        ...
        ik_control = control.Control.create(
            name=f'{self.indexed_name()}_ik', side=self.side(), guide=control_chain[-1],
            match_orient=not ik_world_orient, delete_guide=False, parent=self.controls_group(),
            not_locked_attributes='tr', shape='cube', tag='ik')
        ...
        _, parent_constraint_nodes = api.build_constraint(
            ...
            drivers={
                'targets': (
                    (control_chain[-1].fullPathName(partial_name=True, include_namespace=False), control_chain[-1]),)},
        ...
        reverse_fk_ik = api.factory.create_dg_node(
            name=naming.generate_name(
                [self.indexed_name(), 'fkik'], side=self.side(), suffix='rev'), node_type='reverse')
