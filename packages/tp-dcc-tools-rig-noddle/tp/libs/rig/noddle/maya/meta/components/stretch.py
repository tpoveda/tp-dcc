...
from tp.libs.rig.noddle.maya.functions import nodes
...
class IKSplineStretchComponent(component.Component):
    ...
    def setup(
        ...
        final_scale_mdv = nodes.create(
            'multiplyDivide', [self.component_name(), 'pure'], side=self.side(), suffix='mdv')
        final_scale_mdv.operation.set(2)
        curve_info.arcLength.connect(final_scale_mdv.input1X)

        counter_scale_mdv = nodes.create(
            'multiplyDivide', [self.component_name(), 'scaled'], side=self.side(), suffix='mdv')
        ...
        if switch_control:
            ...
            switch_choice = nodes.create(
                'choice',  [self.component_name(), 'switch'], side=self.side(), suffix='mdl')
