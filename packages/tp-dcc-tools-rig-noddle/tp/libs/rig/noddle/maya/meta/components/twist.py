...
from tp.libs.rig.noddle.maya.functions import attributes, naming, curves, joints
...
class TwistComponent(animcomponent.AnimComponent):
    ...
    def setup(
        ...
        start_locator = api.node_by_name(
            cmds.spaceLocator(
                n=naming.generate_name([self.indexed_name(), 'start'], side=self.side(), suffix='loc'))[0])
        end_locator = api.node_by_name(
            cmds.spaceLocator(
                n=naming.generate_name([self.indexed_name(), 'end'], side=self.side(), suffix='loc'))[0])
    ...
    def start_hook_index(self) -> int:
        """
        Returns this component start hook index.
    ...
    def end_hook_index(self) -> int:
        """
        Returns this component end hook index.
