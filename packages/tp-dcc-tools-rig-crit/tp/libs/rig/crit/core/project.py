...
class Project:
    ...
    def __init__(self, directory: str):
        ...
        self._interface = crit.crit_interface()
    ...
    def meta_data(self) -> Dict:
        ...
        for category in os.listdir(self.path):
            ...
            if path.is_dir(category_path):
                asset_dirs = filter(
                    path.is_dir, [path.join_path(category_path, item) for item in os.listdir(category_path)])
                meta_dict[category] = [path.basename(found_asset) for found_asset in asset_dirs]
