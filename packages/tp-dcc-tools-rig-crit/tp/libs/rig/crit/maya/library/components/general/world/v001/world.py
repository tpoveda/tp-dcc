...
class WorldComponent(component.Component):
    ...
    def post_setup_guide(self):
        ...
        god_node_guide, offset_guide, root_motion_guide = guide_layer.find_guides('godnode', 'offset', 'rootMotion')

        for shape in god_node_guide.iterateShapes():
            vis = shape.visibility
            if vis.isDestination or vis.isLocked:
                continue
            guide_settings.godNodeVis.connect(vis)
    ...
    def setup_rig(self, parent_node: crit.Joint | api.DagNode | None = None):
        ...
        if requires_joint:
            root_joint = self.skeleton_layer().joint(GOD_NODE_ID)
    ...
    def post_setup_rig(self, parent_node: crit.Joint | api.DagNode | None = None):
        output_layer = self.output_layer()
        rig_layer = self.rig_layer()
        outputs = output_layer.find_output_nodes(GOD_NODE_ID, 'offset', 'rootMotion')
        controls = rig_layer.find_controls(GOD_NODE_ID, 'offset', 'rootMotion')
