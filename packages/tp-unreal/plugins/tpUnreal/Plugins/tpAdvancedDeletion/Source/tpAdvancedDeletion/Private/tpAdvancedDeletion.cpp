...
#include "SAdvanceDeletionTab.h"
#include "tpAdvancedDeletionStyle.h"

#define LOCTEXT_NAMESPACE "FtpAdvancedDeletionModule"

void FtpAdvancedDeletionModule::StartupModule()
{
	FtpAdvancedDeletionStyle::Initialize();
	InitContentBrowserExtension();
	RegisterAdvanceDeletionTab();
}

void FtpAdvancedDeletionModule::ShutdownModule()
{
	FGlobalTabmanager::Get()->UnregisterNomadTabSpawner("AdvanceDeletion");
	FtpAdvancedDeletionStyle::Shutdown();
}

void FtpAdvancedDeletionModule::InitContentBrowserExtension()
{
	...
	FContentBrowserMenuExtender_SelectedPaths::CreateRaw(
		this, &FtpAdvancedDeletionModule::CustomContentBrowserMenuExtender
		)
	);
}

TSharedRef<FExtender> FtpAdvancedDeletionModule::CustomContentBrowserMenuExtender(const TArray<FString>& SelectedPaths)
{
	...
	if (SelectedPaths.Num() > 0)
	{
		Extender->AddMenuExtension(
			...
			FMenuExtensionDelegate::CreateRaw(
				this,
				&FtpAdvancedDeletionModule::AddContentBrowserMenuEntry
			)
		);
...
void FtpAdvancedDeletionModule::AddContentBrowserMenuEntry(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(FtpAdvancedDeletionStyle::GetStyleSetName(), "tpAdvancedDeletion.DeleteUnusedAssets"),
		FExecuteAction::CreateRaw(
			this, &FtpAdvancedDeletionModule::OnDeleteUnusedAssetsButtonClicked
		)
	);
	...
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(FtpAdvancedDeletionStyle::GetStyleSetName(), "tpAdvancedDeletion.DeleteEmptyFolders"),
		FExecuteAction::CreateRaw(
			this, &FtpAdvancedDeletionModule::OnDeleteEmptyFoldersButtonClicked
		)
	);
	...
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(FtpAdvancedDeletionStyle::GetStyleSetName(), "tpAdvancedDeletion.AdvancedDeletion"),
		FExecuteAction::CreateRaw(
			this, &FtpAdvancedDeletionModule::OnAdvanceDeletionButtonClicked
		)
	);
}

void FtpAdvancedDeletionModule::OnDeleteUnusedAssetsButtonClicked()
{
...
void FtpAdvancedDeletionModule::OnDeleteEmptyFoldersButtonClicked()
{
	if (FolderPathsSelected.IsEmpty()) return;
...
void FtpAdvancedDeletionModule::OnAdvanceDeletionButtonClicked()
{
	FixUpRedirectors(TArray<FName>({"/Game"}));
...
void FtpAdvancedDeletionModule::FixUpRedirectors(const TArray<FName>& PackagePaths)
{
...
FString FtpAdvancedDeletionModule::MatchAndGetCaptureGroup(const FString& Regex, const FString& Text, const int CaptureGroup)
{
...
TArray<FName> FtpAdvancedDeletionModule::GetTopLevelPackagePath(const TArray<FAssetData>& Array)
{
...
void FtpAdvancedDeletionModule::RegisterAdvanceDeletionTab()
{
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
		"AdvanceDeletion",
		FOnSpawnTab::CreateRaw(
			this,
			&FtpAdvancedDeletionModule::OnSpawnAdvanceDeletionTab
		))
	.SetDisplayName(LOCTEXT("AdvanceDeletionTabName", "Advance Deletion"))
	.SetIcon(FSlateIcon(FtpAdvancedDeletionStyle::GetStyleSetName(), "tpAdvancedDeletion.AdvancedDeletion"));
	
}

TSharedRef<SDockTab> FtpAdvancedDeletionModule::OnSpawnAdvanceDeletionTab(const FSpawnTabArgs& Args)
{
...
TArray<TSharedPtr<FAssetData>> FtpAdvancedDeletionModule::GetAllAssetsDataUnderSelectedFolder()
{
...
bool FtpAdvancedDeletionModule::DeleteSingleAssetForAssetList(const FAssetData& AssetDataToDelete)
{
	if (ObjectTools::DeleteAssets({AssetDataToDelete}) > 0) return true;
	return false;
}

bool FtpAdvancedDeletionModule::DeleteMultipleAssetsForAssetList(const TArray<FAssetData>& AssetDataToDelete)
{
	if (ObjectTools::DeleteAssets(AssetDataToDelete) > 0) return true;
	return false;
}

void FtpAdvancedDeletionModule::ListUnusedAssetsForAssetList(const TArray<TSharedPtr<FAssetData>>& AssetsDataToFilter,
...
void FtpAdvancedDeletionModule::ListSameNameAssetsForAssetList(const TArray<TSharedPtr<FAssetData>>& AssetsDataToFilter,
...
void FtpAdvancedDeletionModule::SyncToClickedAssetForAssetList(const FString& AssetPathToSync)
{
...
IMPLEMENT_MODULE(FtpAdvancedDeletionModule, tpAdvancedDeletion)